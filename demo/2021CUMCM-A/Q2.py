...
def mask(coord):
    ...
    return np.linalg.norm(ortho) <= 150.0 + 10  # 150米半径, 10米容差
...
vertex_id = np.argmin(np.einsum("ij,j->i", coords0, n_s))
vertex = coords0[vertex_id]
focus = -(R - f) * n_s  # 焦点
...
def get_adjusted_points(deltas):
    ...
    '''
    # 所有原点指向的单位向量 (从球心到节点)
    r0_norm = np.linalg.norm(coords0, axis=1, keepdims=True)
    norm_dir = coords0 / r0_norm
    coords_new = coords0 + norm_dir * deltas[:,None]
    return coords_new
...
# 预计算原始距离
d_orig = np.array([np.linalg.norm(coords0[i]-coords0[j]) for i,j in edge_list])
...
if __name__ == "__main__":
    ...
    out_df = pd.DataFrame({'对应主索节点编号': node_id_list, '伸缩量（米）': np.round(x0,3)})
