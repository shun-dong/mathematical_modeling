...
if nargin<1||(isempty(filename)&&~ischar(filename)), 
    ...
    elseif strcmp(answ,'spmT_ file')
        conn_disp('Select contrast file(s) (spmT). Select a single file containing surface-vertex statistics for both hemispheres; or two files each containing left&right hemisphere statistics, respectively');
...
if ismember([file_name,file_ext],{'SPM.mat','spm.mat'})
    ...
    conn_disp(['reading analysis info from ',filename]);
    load(filename,'SPM');
...
elseif ismember(file_ext,{'.img','.nii'})        
    spmfilename=[];
    conn_disp(['reading T-statistics from ',filename]);
    a=spm_vol(filename);
    SPM=[];
...
if ~isempty(dir(fullfile(file_path,'mask.img')))
    conn_disp(['reading analysis mask from ',fullfile(file_path,'mask.img')]);
...
if isstruct(a)
    try
        ...
        conn_disp('warning: no dof information found. Using inf degrees of freedom');
        dof=inf;
    end
    T=spm_read_vols(a);
else
...
if any(strcmp(varargin(1:2:end-1),'export_mask')), conn_surf_results_doexport(varargin{2*find(strcmp(varargin(1:2:end),'export_mask'),1)}); end
    ...
    function conn_surf_results_update(varargin)
        ...
        elseif isequal(size(color_brain),[size(CURV{hem},1),3])
            ...
            conn_disp('unrecognized brain surface color option');
...
%         uicontrol('style','pushbutton','units','norm','position',[0,.95,.1,.05],'string','Print','callback',@conn_surf_results_doprint,'tooltipstring','Print high-resolution images to file');
        ...
        function conn_surf_results_export(tfilename,varargin)
            ...
            fclose(fh);
            %conn_disp(['Created file ',fullfile(tpathname,[tfilename,'.rgb'])]);
            ...
            fclose(fh);
            %conn_disp(['Created file ',fullfile(tpathname,[tfilename,'.stats'])]);
        end
    end
    ...
    function conn_surf_results_print(varargin)
        ...
        if ~isempty(answ)
            ...
            try
                ...
                conn_disp('Error: Failed to print file');
            end
            if ok
                try
                    a=imread(tfilename);
                    ...
                    conn_disp(['Saved file: ',tfilename]);
                end
            end
        end
        DOPRINT='none';
    ...
    function ok=conn_surf_results_randomise(varargin)
        if isempty(SPM), conn_disp('randomization analyses only available when selecting SPM.mat file. Please re-run analyses using conn_module(''glm'',...) syntax'); ok=false; return; end
        ...
        if ishandle(fh), 
            ...
            if v3==1, % now
                ht=conn_msgbox('Preparing data. Please wait...','conn_surf_results',-1);
                try
