...
elseif nargin&&any(strcmp(varargin(cellfun(@ischar,varargin)),'flfolders')),  % each conn folder contains a different subject
    if ischar(varargin{1})&&isdir(varargin{1}), qafolder=varargin{1};
    else qafolder=pwd;
    end
    qagroups=conn_sortfilenames(conn_dir(fullfile(qafolder,'QA_*'),'-dir','-cell'));
...
elseif nargin&&ischar(varargin{1})&&isdir(varargin{1})                        % single QA report in specified folder
                ...
                switch(dlg.uanalysestype(dlg.ianalysis))
                    case 1,
                        ...
                        if numel(in)==1,
                            ...
                            set(dlg.handles.text2,'string',tpath(numel(qafolder)+1:end),'visible','on');
                    ...
                    case {2,3,4} % QA_DENOISE
                        ...
                        if ~isempty(htlegend)
                            if dlg.uanalysestype(dlg.ianalysis)==4,
                                try, ht=legend(htlegend,{sprintf('expected mean%cstd under Null Hypothesis (if no QC-FC associations exist at any distance level)',177)},'location','northwest'); set(ht,'box','off','fontsize',5+font_offset); catch, legend(ht,'randomised reference'); end
                            else
                                try, ht=legend(htlegend,{'expected shape of distribution under Null Hypothesis (if no QC-FC associations exist)'},'location','northwest'); set(ht,'box','off','fontsize',5+font_offset); catch, legend(ht,'randomised reference'); end
                            end
                        end
