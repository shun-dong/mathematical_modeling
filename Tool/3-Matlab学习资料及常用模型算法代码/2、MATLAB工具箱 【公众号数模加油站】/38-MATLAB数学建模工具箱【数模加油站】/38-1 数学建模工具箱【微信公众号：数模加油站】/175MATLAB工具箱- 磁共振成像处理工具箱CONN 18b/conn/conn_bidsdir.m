...
if isstruct(filenames)
    ...
    for n=1:2:numel(varargin)-1
        ...
        if ~isempty(temp)&&~any(temp)&&~isempty(varargin{n+1}) % special case regexp wildcards
        	if ischar(varargin{n+1})&&any(varargin{n+1}=='*'), temp=select&cellfun('length',regexp(dataset.data.(varargin{n}),varargin{n+1}));
    ...
    files_all=conn_dir(fullfile(filenames,'sub-*'),'-cell');
    ...
    [files_all,nill,idx]=unique(files_all); isgz=accumarray(idx(:),isgz(:),[],@min)>0; % note: disregards [filename.ext].gz if [filename.ext] exists
    ...
    dataset.data.series=regexprep(cellfun(@(a,b,c)[a,b,c],files_parts(:,find(ismember(spec,'ses'),1)),files_parts(:,find(ismember(spec,'task'),1)),files_parts(:,find(ismember(spec,'run'),1)),'uni',0),'^_+','');
    for n=1:numel(spec),
        dataset.data.(spec{n})=regexprep(files_parts(:,n),['^_?',spec{n},'-'],'');
    end
end
for n=[{'file','contents','folder','format','series'},spec] 
