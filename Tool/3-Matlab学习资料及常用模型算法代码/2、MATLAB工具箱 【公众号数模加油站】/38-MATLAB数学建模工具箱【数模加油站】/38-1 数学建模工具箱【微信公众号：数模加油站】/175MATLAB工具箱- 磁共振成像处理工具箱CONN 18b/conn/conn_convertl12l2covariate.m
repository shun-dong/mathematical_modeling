...
measures={ 'average',           @(x,w,dim)sum(x.*w,dim)./max(eps,sum(w,dim)),   @(x,dim,k1,k2,k3)['\frac{1}{',k3,'}\sum_{',dim,'=',k1,'}^{',k2,'} ',x];
           ...
           'user-defined',      @(x,w,dim)userdefined(x,w,dim),                 @(x,dim,k1,k2,k3)['f_',k2,'\{ ',x,' \}'];
...
if nargin==1, % clear
    ...
    if numel(f)<dim||isempty(f{dim})
        ...
        answ=inputdlg(sprintf('user-defined function aggregating across %s',str{dim}),'',1,{sprintf('mean(x,%d)',dim)});
