...
try, 
    if max(abs(1-x0*(pinv(x0)*ones(size(x0,1),1))))>1e-6, 
        if numel(CONN_h.menus.m_results.design.designmatrix_name)==1, tstr={['WARNING: missing constant term (suggestion: select ''AllSubjects'' and ''',CONN_h.menus.m_results.design.designmatrix_name{1},''''],[' in the subject-effects list and then select the ''effect of ',CONN_h.menus.m_results.design.designmatrix_name{1},''' contrast)']};
        else tstr={'WARNING: possibly incomplete model: no constant term modeled','(suggestion: select also ''AllSubjects'' in the subject-effects list)'};
        end 
