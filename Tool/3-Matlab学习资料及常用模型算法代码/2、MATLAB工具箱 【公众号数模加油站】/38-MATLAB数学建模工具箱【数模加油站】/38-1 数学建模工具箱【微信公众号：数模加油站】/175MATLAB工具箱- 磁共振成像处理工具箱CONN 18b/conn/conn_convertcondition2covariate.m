    ...
    for nsub=subjects(:)',
        ...
        for nses=1:nsess,
            ...
            if numel(CONN_x.Setup.nscans)>=nsub&&numel(CONN_x.Setup.nscans{nsub})>=nses
                x=zeros(offs+ceil(CONN_x.Setup.nscans{nsub}{nses}*RT{nsub}{nses}/rt),1);
                if length(durat)>=1, 
                    for n1=1:length(onset), 
                        tdurat=max(rt,min(offs*rt+RT{nsub}{nses}*CONN_x.Setup.nscans{nsub}{nses}-onset(n1),durat(min(length(durat),n1))));
