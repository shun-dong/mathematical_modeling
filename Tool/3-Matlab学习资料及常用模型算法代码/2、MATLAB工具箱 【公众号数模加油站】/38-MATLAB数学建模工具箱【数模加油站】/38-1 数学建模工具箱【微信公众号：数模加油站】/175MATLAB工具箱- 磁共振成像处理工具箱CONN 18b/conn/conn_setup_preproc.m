...
%     default_mnifield                      : same as default_mni but with vdm/fieldmap information
...
%     functional_realign&unwarp&fieldmap    : functional realignemnt & unwarp & distortion correction (functional_realign&unwarp + corrects static inhomogeneity distortions)
...
%      vdm_et1         : (functional_vdm_create only) ET1 (Echo Time first echo in fieldmap sequence) (default [] : attempt to obtain this value from sidecar .json file / BIDS)
%      vdm_et2         : (functional_vdm_create only) ET2 (Echo Time second echo in fieldmap sequence) (default [] : attempt to obtain this value from sidecar .json file / BIDS)
%      vdm_ert         : (functional_vdm_create only) ERT (EPI Total Readout Time in funcional data) (default [] : attempt to obtain this value from sidecar .json file / BIDS)
...
options=varargin;
steps={'default_mni','default_mnifield','default_ss','default_ssfield','default_ssnl',...
...
steps_names={'<HTML><b>default preprocessing pipeline</b> for volume-based analyses (direct normalization to MNI-space)</HTML>','<HTML><b>preprocessing pipeline</b> for volume-based analyses (indirect normalization to MNI-space) when FieldMaps are available</HTML>','<HTML><b>preprocessing pipeline</b> for surface-based analyses (in subject-space)</HTML>','<HTML><b>preprocessing pipeline</b> for surface-based analyses (in subject-space) when FieldMaps are available</HTML>','<HTML><b>preprocessing pipeline</b> for surface-based analyses (in subject-space) using nonlinear coregistration</HTML>',...
    ...
    'structural Normalization (MNI space normalization)','structural Segmentation & Normalization (simultaneous Grey/White/CSF segmentation and MNI normalization)',...
    ...
    'functional Realignment & unwarp & distortion correction (subject motion estimation and correction)','functional Outlier detection (ART-based identification of outlier scans for scrubbing)','functional Direct Coregistration to structural (rigid body transformation)',...
    ...
    'functional Label current functional files as part of list of Secondary Datasets (user-defined label)', ...
    'functional Label current functional files as part of list of Secondary Datasets ("original data" label)', ...
    'functional Label current functional files as part of list of Secondary Datasets ("subject-space data" label)', ...
    'functional Label current functional files as part of list of Secondary Datasets ("mni-space data" label)', ...
    'functional Label current functional files as part of list of Secondary Datasets ("surface-space data" label)', ...
    ...
    'functional Creation of voxel-displacement map (VDM) for distortion correction' ...
    };
...
steps_descr={{'INPUT: structural&functional volumes','OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Grey/White/CSF normalized masks, realigned slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&VDM volumes','OUTPUT (all in MNI-space): skull-stripped normalized structural volume, Grey/White/CSF normalized masks, realigned&unwarp slice-time corrected normalized smoothed functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional&VDM volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned&unwarp slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},{'INPUT: structural&functional volumes','OUTPUT (all in subject-space): skull-stripped structural volume, Grey/White/CSF masks, realigned slice-time corrected coregistered functional volumes, subject movement ''realignment'' and ''scrubbing'' 1st-level covariate'},...
    ...
    {'INPUT: structural volume','OUTPUT: skull-stripped normalized structural volume (in MNI space)'},{'INPUT: structural volume','OUTPUT: skull-stripped normalized structural volume, normalized Grey/White/CSF masks (all in MNI space)'},...
    ...
    {'INPUT: functional volumes; double-echo FieldMap acquisition files (e.g. Magnitude+PhaseDiff volumes) in "fmap" dataset','OUTPUT: SPM vdm files in "vdm" dataset'} ...
    };
...
if ~isempty(STEPS)&&(ischar(STEPS)||(iscell(STEPS)&&numel(STEPS)==1))
    STEPS=regexprep(char(STEPS),{'^default_mniphase$','^default_ssphase$'},{'default_mnifield','default_ssfield'});
...
if ~nargin||isempty(STEPS)||dogui,
    ...
    if ~isempty(typeselect)
        switch(typeselect)
            case 'structural', idx=find(cellfun('length',regexp(steps,'^structural')));
            case 'functional', idx=find(cellfun('length',regexp(steps,'^functional')));
        end
    end
    steps=steps(idx);
...
sliceorder_select_options_extended={'ascending (e.g. 1,2,3,4,5)','descending (e.g. 5,4,3,2,1)','interleaved middle-top (e.g. 3,5,2,4,1)','interleaved bottom-up (e.g. 1,3,5,2,4)','interleaved top-down (e.g. 5,3,1,4,2)','interleaved (Siemens) (e.g. 1,3,5,2,4)','BIDS (from functional .json metadata)'};
if any(ismember('functional_slicetime',lSTEPS))
    ...
    if isempty(sliceorder)&&isempty(sliceorder_select)
        [sliceorder_select,tok] = listdlg('PromptString','Select slice order:','ListSize',[300 200],'SelectionMode','single','ListString',[sliceorder_select_options_extended,{'manually define','do not know (skip slice timing correction)'}]);
...
if dogui&&any(ismember(lSTEPS,{'functional_vdm_create'}))
    ...
    ht7a=uicontrol('style','text','units','norm','position',[.1,.2,.6,.1],'string','Blip direction (+1 or -1)','horizontalalignment','left','backgroundcolor',1*[1 1 1]);
    if isempty(vdm_blip), tvdm_blip=-1; else tvdm_blip=vdm_blip; end
    ht7=uicontrol('style','edit','units','norm','position',[.7,.2,.2,.1],'string',num2str(tvdm_blip),'tooltipstring','defines k-space traversal blip direction (+1 for positive direction, -1 for negative direction)');
...
if dogui&&any(ismember(lSTEPS,{'structural_normalize','structural_segment&normalize','structural_segment','functional_normalize','functional_segment&normalize','functional_segment','functional_segment&normalize_direct','functional_segment&normalize_indirect','functional_normalize_indirect','functional_normalize_direct','structural_manualspatialdef','functional_manualspatialdef'}))
    ...
    if ~any(ismember(lSTEPS,{'functional_normalize','functional_segment&normalize','functional_segment','functional_segment&normalize_direct','functional_normalize_direct','functional_segment&normalize_indirect','functional_normalize_indirect','functional_manualspatialdef'})), set([ht2a ht2 ht5],'enable','off'); end
...
if any(ismember('functional_surface_smooth',lSTEPS))
    if isempty(diffusionsteps)
        diffusionsteps=inputdlg('Enter number of diffusion steps for smoothing','conn_setup_preproc',1,{num2str(10)});
...
if any(ismember('functional_label',lSTEPS))
    if isempty(label)
        ...
        if nl>1, label=inputdlg(repmat({'Enter functional label'},1,nl),'conn_setup_preproc',1,arrayfun(@(n)sprintf('Label%d',n),1:nl,'uni',0));
        else label=inputdlg('Enter functional label  (arbitrary description)','conn_setup_preproc',1,{datestr(now)});
        end
...
if parallel_N>0,
    ...
    info=conn_jobmanager('submit','setup_preprocessing',subjects,parallel_N,[],...
        STEPS,...
        'sessions',sessions,'sets',sets,'fwhm',fwhm,'label',label,'sliceorder',sliceorder,'ta',ta,'unwarp',unwarp,'removescans',removescans,'applytofunctional',applytofunctional,...
        ...
        'doimport',doimport,'dogui',0,'functional_template',functional_template,'structural_template',structural_template,...
        'affreg',affreg,'tpm_template',tpm_template,'tpm_ngaus',tpm_ngaus,'vdm_et1',vdm_et1,'vdm_et2',vdm_et2,'vdm_ert',vdm_ert,'vdm_blip',vdm_blip,'vdm_type',vdm_type);
    ...
    CONN_x.SetupPreproc.log{end+1}={'timestamp',datestr(now),'ver_CONN',conn('ver'),'ver_SPM',spm('version'),'steps',...
        STEPS,...
        'subjects',subjects,'sessions',sessions,'sets',sets,'fwhm',fwhm,'label',label,'sliceorder',sliceorder,'sliceorder_select',sliceorder_select,'ta',ta,'unwarp',unwarp,'removescans',removescans,'applytofunctional',applytofunctional,...
        'coregtomean',coregtomean,'rtm',rtm,'coregsource',coregsource,'reorient',reorient,'respatialdef',respatialdef,'art_thresholds',art_thresholds,'voxelsize_anat',voxelsize_anat,'voxelsize_func',voxelsize_func,'boundingbox',boundingbox,'interp',interp,'diffusionsteps',diffusionsteps,...
        'doimport',doimport,'dogui',0,'functional_template',functional_template,'structural_template',structural_template,...
        'affreg',affreg,'tpm_template',tpm_template,'tpm_ngaus',tpm_ngaus,'vdm_et1',vdm_et1,'vdm_et2',vdm_et2,'vdm_ert',vdm_ert,'vdm_blip',vdm_blip,'vdm_type',vdm_type};
end
...
for iSTEP=1:numel(STEPS)
                ...
                for nses=1:nsess
                    if ismember(nses,sessions)
                        ...
                        elseif 1, % note: convert to slice-timing (ms) syntax for SPM                            
                            matlabbatch{end}.spm.temporal.st.so=(sliceorder-1)*matlabbatch{end}.spm.temporal.st.ta/(nslice-1);
                            matlabbatch{end}.spm.temporal.st.refslice=mean(matlabbatch{end}.spm.temporal.st.so); % slice timing (ms)
                        else % keep slice-order syntax
                            matlabbatch{end}.spm.temporal.st.so=sliceorder;
            ...
            for isubject=1:numel(subjects),
                ...
                for nses=1:nsess
                    if ismember(nses,sessions)
                        ...
                        outputfiles{isubject}{nses}=conn_prepend('m',char(temp));
                    ...
                    if ismember(nses,sessions)
                        ...
                        if ~isempty(fmap)
                            ...
                            if isequal(vdm_type,1)||(isempty(vdm_type)&&(numel(fmap)==2||numel(fmap)==3)), % Magnitude1+PhaseDiff or Magnitude1+Magnitude2+PhaseDiff 
                                ...
                                if nses==1&&isempty(ET1), conn_disp('fprintf','warning: unable to find EchoTime2 or EchoTimeDifference information in %s\n',fmap{1}); end
                            ...
                            elseif isequal(vdm_type,3)||(isempty(vdm_type)&&numel(fmap)==1), % FieldMap [note: work in progress; needs further testing]
                                ...
                                else conn_disp('fprintf','units of FieldMap %s not found, assuming Hz\n',filename);
                                end
                                if isempty(ERT), ERT=1000*conn_jsonread(filename,'TotalReadoutTime'); end
                                if isempty(ERT), ERT=1000./conn_jsonread(filename,'BandwidthPerPixelPhaseEncode'); end
                                if nses==1&&isempty(ERT), conn_disp('fprintf','warning: unable to find TotalReadoutTime or BandwidthPerPixelPhaseEncode information in %s\n',filename); end
    ...
    if strncmp(lower(STEP),'interactive_',numel('interactive_'))
        ...
        elseif any(strcmpi(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),{'functional_removescans','functional_manualorient','structural_manualorient','functional_center','functional_centertostruct','structural_center','functional_motionmask','functional_label','functional_label_as_original', 'functional_label_as_subjectspace', 'functional_label_as_mnispace', 'functional_label_as_surfacespace', 'functional_label_as_smoothed','functional_surface_smooth','functional_surface_resample','functional_vdm_create'}))
        ...
        elseif any(strcmpi(regexprep(lower(STEP),'^run_|^update_|^interactive_',''),{'functional_removescans','functional_manualorient','structural_manualorient','functional_center','functional_centertostruct','structural_center','functional_motionmask','functional_label','functional_label_as_original', 'functional_label_as_subjectspace', 'functional_label_as_mnispace', 'functional_label_as_surfacespace', 'functional_label_as_smoothed'}))
...
if ~nargin, hdl=gcbo; end
dlg=get(hdl,'userdata');
val=listdlg('liststring',dlg.steps_names(dlg.steps_order),'selectionmode','single','promptstring',{'Select pipeline (in bold) or individual preprocessing step (in regular font)'},'ListSize',[700 700]);
if ~isempty(val)
