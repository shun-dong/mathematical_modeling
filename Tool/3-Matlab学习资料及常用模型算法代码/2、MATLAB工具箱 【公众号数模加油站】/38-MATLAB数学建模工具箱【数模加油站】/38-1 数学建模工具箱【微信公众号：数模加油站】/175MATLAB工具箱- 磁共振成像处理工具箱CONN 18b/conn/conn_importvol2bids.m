function [out,changed,nV]=conn_importvol2bids(filename,nsub,nses,fmod,ftype,docopy,dodisp,dosinglesubjectreduce)
...
elseif ismember(fmod,{'func','anat','dwi','fmap'}), BIDSfolder=fullfile(CONN_x.folders.bids,'dataset');  % root directory
...
if docopy
    ...
    switch(fmod) % initializes CONN_x functional/structural fields
        case 'func'
            conn_set_functional(nsub(end),nses(end),nset,out); % note: when nsubs = [nsubs1 nsubs2] or nses = [nses1 nses2] file_bids(nsub1,nses1) is assigned to nsub2,nses2 (in order to allow subject- and session- independent data)
%             if ~nset, 
