    ...
    if mirrorscreen||savelog||nargout>0
        ...
        if ~isempty(newstr)
            ...
            if savelog&&~isempty(str)
                ...
                fclose(fh);
                if mirrorscreen&&~isequal(lastlogfile,filename), set(CONN_h.screen.hlog,'name',sprintf('CONN log history (%s)',filename)); end
                ...
                lastlogfile=filename;
            elseif ~savelog&&mirrorscreen, set(CONN_h.screen.hlog,'name','CONN log history'); lastlogfile=''; 
            end
...
elseif mirrorscreen
    set(CONN_h.screen.hlogstr,'backgroundcolor',CONN_gui.backgroundcolor,'foregroundcolor',CONN_gui.fontcolorB);
end
%end
end
