function SPM = conn_module_glm(X,filenames,C1,C2,swd,effectnames,contrastnames,secondlevelanalyses,maskfile)
...
if ~isequal(C2,eye(Ny)) % two-step procedure for SPM
    clear VY;
    nw=0;
    for nr=reshape(find(any(C2,2)),1,[]),
        ...
        for nsub=1:Ns
            ...
            for nc=reshape(find(C2(nr,:)),1,[])
                b=b+C2(nr,nc)*spm_read_vols(SPM.xY.VY(nsub,nc));
            end
            a=SPM.xY.VY(nsub,1);
            a.fname=sprintf('data_%04d_%04d.nii',nsub,nw);
            a.pinfo=[1;0;0];
...
if nargin<6||isempty(effectnames), effectnames=arrayfun(@(n)sprintf('effect %d',n),1:Nx,'uni',0); end
...
if issurface||ismember(secondlevelanalyses,[1 3]) % nonparametric stats
    ...
    for n2=1:SPM.xY.VY(1).dim(1)
        ...
        if ~isempty(maskfile), maskthis=maskthis&spm_get_data(vmaskfile,xyz)>0; end
...
elseif ismember(secondlevelanalyses,[1 2]) % volume-based parametric stats
    ...
    spm('Defaults','fmri');
    spm_unlink('mask.img','mask.hdr','mask.nii');
    if ~isempty(maskfile), SPM.xM=struct('T',[],'TH',-Inf(size(SPM.xX.X,1),1),'I',0,'VM',{vmaskfile},'xs',struct('Masking','analysis threshold')); end
    SPM=spm_spm(SPM);
    c=kron(C2,C1); 
