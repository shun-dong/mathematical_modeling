function [f] = ObjFun_Schaffers_f6(position_matrix, num_particles_2_evaluate)
% 2D only
...
% Initialization space: Assuming symmetric initialization, positions
    %were initialized to lie within [-100, 100] when the
    %position matrix was created.  This can be changed within
    %"Objectives.m."

xSPyS = sum(position_matrix(1:num_particles_2_evaluate,:).^2, 2); %Calculate position_matrix^2 + y^2 or x1^2 + x2^2.
f = 0.5 + ((sin(sqrt(xSPyS))).^2 - 0.5)./((1 + xSPyS/1000).^2);