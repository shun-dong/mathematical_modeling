...
%  CREMEZ(N,F,'highpass', ...) designs a linear-phase highpass filter
%  response using CREMEZ.
%
%  CREMEZ(N,F,{'highpass', D}, ...) specifies group-delay offset D
...
%    b = cremez(30,[-1 -.5 -.4 .7 .8 1],'highpass');
%    freqz(b,1,512,'whole');
%
%  EXAMPLE: Reduced group delay filter response:
%    b = cremez(30,[0 .6 .7 1],{'highpass',-1});
%
%  See also CREMEZ.
...
%   $Revision: 1.6 $  $Date: 1997/11/26 20:14:23 $
...
if (Le == 4),
  if any(F < 0),
    error('Band edges must be non-negative for 2-band Highpass designs.'); 
  end
elseif (Le == 6),
  if F(3)*F(4) > 0,
    error('Stopband must include DC for 3-band Highpass designs.');
  end
else
  error('There must be either 4 or 6 band edges for Highpass designs.')
end
