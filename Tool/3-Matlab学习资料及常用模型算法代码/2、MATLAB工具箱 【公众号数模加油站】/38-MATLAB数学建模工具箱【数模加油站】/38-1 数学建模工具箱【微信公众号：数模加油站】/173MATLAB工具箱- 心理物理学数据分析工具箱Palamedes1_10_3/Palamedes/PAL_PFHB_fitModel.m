...
%       in Windows PATH variable). This process may fail to find Stan or 
%       JAGS or may find a version that is not the latest installed. To 
%       specify path yourself use this argument (e.g., 'enginepath') 
%       followed by the path to directory (e.g., '/opt/cmdstan-2.18.1').
%
...
if strcmp(pfhb.engine.engine,'stan')
     if pfhb.engine.recyclestan && ((exist('stanModel','file') && any(strcmpi(pfhb.machine.machine,{'MACI64','GLNXA64'}))) || (exist('stanModel.exe','file') && strcmpi(pfhb.machine.machine,'PCWIN64')))
        ...
        if status ~= 0
            ...
            message = [message, 'First thing to try: Can you get the CmdStan bernoulli example (see CmdStan User''s guide) to work? '];
            message = [message, 'If yes: This is possibly a Palamedes bug. Send us an e-mail: palamedes@palamedestoolbox.org. If no: Get bernoulli example to work, then try again.']; 
