function [indfit,indres]=testind(indstr,params);
%TESTIND    Evaluates a GPLAB individual on a different data set.
%   [FITNESS,RESULT]=TESTIND(IND,PARAMS) requests the names of the
%   test data files (input and desired output, or just input for
%   predictions), evaluates an individual (IND) on this new data
%   set, and returns both the FITNESS of the individual and the
%   results obtained. The fitness may not be returned, in case we
%   are doing only predictions (no desired output file provided).
%
%   Input arguments:
%      IND - the individual to evaluate on the new data set (string)
%      PARAMS - the algorithm running parameters (struct)
%   Output arguments:
%      FITNESS - the fitness of IND on the new dataset (double, or string)
%      RESULT - the results obtained by IND on the new dataset (matrix)
%
...
% now calc fitness:
if ~isempty(params.datafiley)
   [indfit,indres]=feval(params.calcfitness,indstr,params,data,state.terminals,state.varsvals);
else
   [indfit,indres]=feval(params.calcfitness,indstr,params,data,state.terminals,state.varsvals);
