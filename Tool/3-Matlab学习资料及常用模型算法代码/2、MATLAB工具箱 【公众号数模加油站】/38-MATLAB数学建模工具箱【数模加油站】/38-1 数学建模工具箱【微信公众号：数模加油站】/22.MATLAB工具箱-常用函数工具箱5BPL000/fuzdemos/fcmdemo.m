...
%   $Revision: 1.9 $  $Date: 1997/12/01 21:44:17 $
...
elseif strcmp(action, 'set_gui'),   % set figure, axes and gui's
    % ====== setting figure
    FcmFigH = figure('Name', FcmFigTitle, 'NumberTitle', 'off');
...
elseif strcmp(action, 'start_clustering'),
    ...
    expo = str2num(get(findobj(FcmFigH, 'tag', 'exponent'), 'string'));
    cluster_n = get(findobj(FcmFigH, 'tag', 'cluster_number'), 'value')+1;
    max_iter = str2num(get(findobj( ...
        FcmFigH, 'tag', 'max_iter'), 'string'));
    min_eps = str2num(get(findobj( ...
