...
%   $Revision: 1.17 $  $Date: 1997/12/01 21:45:24 $
...
if strcmp(ruleFormat,'verbose') | strcmp(ruleFormat,'symbolic'),
    ...
    for n=1:length(ruleIndex),
        ...
        for inCount=1:numInputs,
            ...
            MFIndex=rule(inCount);
            if MFIndex~=0,
                MFIndex2=sum(numInputMFs(1:(inCount-1)))+abs(MFIndex);
                if MFIndex>0,
                    % MF use is normal
                    ruleStr1=[ruleStr1, ...
                        lftParen,deblank(inLabels(inCount,:)),equalStr, ...
                        deblank(inMFLabels(MFIndex2,:)),rtParen];
                else
        ...
        for outCount=1:numOutputs,
            ...
            if rule(outCount+numInputs)~=0,
                MFIndex=sum(numOutputMFs(1:(outCount-1)))+rule(numInputs+outCount);
                ...
                if MFIndex>0,
                    ...
                    ruleStr2=[ruleStr2, ...
                        ...
                        deblank(outMFLabels(abs(MFIndex),:)),rtParen];
                end
