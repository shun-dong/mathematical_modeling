...
case '#add';
   ...
   if errflag==0;
    ...
    if isempty(str2num(weight))
       ...
       wght=max(min(str2num(weight), 1), 0);
    end
    ...
    if endedge>maxRight-left
      %out of border
       ...
       for i=1:numOutputs
          ...
          if isfield(fis.output(outIndex-1), 'mf') & ~isempty(fis.output(outIndex-1).mf)
            str=getfis(fis, 'output', outIndex-1, 'mflabels');
            str=strvcat(str, 'none');
          else
            str=[];
          end
          ...
          textHndl=uicontrol( ...
           ...
           'Position',textPos, ...
           'BackgroundColor',frmColor, ...
           'String', [fis.output(outIndex-1).name ' is'],...
           'HorizontalAlignment','center', ...
           'Tag',name);

           textPos=rulePos+[0, boxHeight+spacing+btnHt*2/3, 0, btnHt*2/3-boxHeight];
           name=['ruleoutkeyw' num2str(outIndex-1)];
           if numOutputs==i
            strname='Then';
           else
            strHndl=findobj(gcf, 'Tag', 'radio');
