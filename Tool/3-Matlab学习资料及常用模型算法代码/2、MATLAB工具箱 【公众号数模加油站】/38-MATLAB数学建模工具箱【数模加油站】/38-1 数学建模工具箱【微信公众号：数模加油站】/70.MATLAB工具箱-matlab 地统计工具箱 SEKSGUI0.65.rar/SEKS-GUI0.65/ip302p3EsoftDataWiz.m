function varargout = ip302p2EsoftDataWiz(varargin)
%IP302P2ESOFTDATAWIZ M-file for ip302p2EsoftDataWiz.fig
%      IP302P2ESOFTDATAWIZ, by itself, creates a new IP302P2ESOFTDATAWIZ or raises the existing
%      singleton*.
%
%      H = IP302P2ESOFTDATAWIZ returns the handle to a new IP302P2ESOFTDATAWIZ or the handle to
%      the existing singleton*.
%
%      IP302P2ESOFTDATAWIZ('Property','Value',...) creates a new IP302P2ESOFTDATAWIZ using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to ip302p2EsoftDataWiz_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      IP302P2ESOFTDATAWIZ('CALLBACK') and IP302P2ESOFTDATAWIZ('CALLBACK',hObject,...) call the
%      local function named CALLBACK in IP302P2ESOFTDATAWIZ.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help ip302p2EsoftDataWiz

% Last Modified by GUIDE v2.5 13-Jun-2005 11:19:15

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ip302p2EsoftDataWiz_OpeningFcn, ...
                   'gui_OutputFcn',  @ip302p2EsoftDataWiz_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before ip302p2EsoftDataWiz is made visible.
function ip302p2EsoftDataWiz_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for ip302p2EsoftDataWiz
...
% UIWAIT makes ip302p2EsoftDataWiz wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% Create soft data types pictures
axes(handles.sdAsoftEaxes)
image(imread('guiResources/soft4ex1.png'));
axis off
axes(handles.sdBsoftEaxes)
image(imread('guiResources/soft4ex2.png'));
axis off



% --- Outputs from this function are returned to the command line.
function varargout = ip302p2EsoftDataWiz_OutputFcn(hObject, eventdata, handles)
...
if ~ischar(sdFilename)  % If the user has not provided a soft data file
    ...
    softDataPresent = 1;
    allOk = ip302p3EsoftDataFunction(handles);            
    if allOk
...
function [allOk] = ip302p3EsoftDataFunction(handles)
...
softpdftype = 4;                % BMELIB parameter based on current (4) SD type
...
if isempty(xlsfinfo(sdFilename))  % If it is not Excel format, expect an ASCII file
  ...
  while ~feof(fid)                % Start reading the file to its end
    ...
    if str~=-1                    % If this is not the end of line
      ...
      nLimi = 3;                        % Number of limi values needed for type E SD
      nProbdens = nBins+1;              % Number of required PD values for type E SD
      ...
      if nTempval~=expectednTempval     % If line contains unexpected number of values
        ...
        % Since nLimi=3 we expand the limi matrix and change the softpdftype from 4 to 2.
        ...
        softpdftype = 2;                % Redefine BME softpdftype
...
% Here, we have softpdftype=4, therefore the proper way to check the area under 
