function varargout = ip302p3CsoftDataWiz(varargin)
%IP302P3CSOFTDATAWIZ M-file for ip302p3CsoftDataWiz.fig
%      IP302P3CSOFTDATAWIZ, by itself, creates a new IP302P3CSOFTDATAWIZ or raises the existing
%      singleton*.
%
%      H = IP302P3CSOFTDATAWIZ returns the handle to a new IP302P3CSOFTDATAWIZ or the handle to
%      the existing singleton*.
%
%      IP302P3CSOFTDATAWIZ('Property','Value',...) creates a new IP302P3CSOFTDATAWIZ using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to ip302p3CsoftDataWiz_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      IP302P3CSOFTDATAWIZ('CALLBACK') and IP302P3CSOFTDATAWIZ('CALLBACK',hObject,...) call the
%      local function named CALLBACK in IP302P3CSOFTDATAWIZ.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help ip302p3CsoftDataWiz

% Last Modified by GUIDE v2.5 13-Jun-2005 11:16:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ip302p3CsoftDataWiz_OpeningFcn, ...
                   'gui_OutputFcn',  @ip302p3CsoftDataWiz_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before ip302p3CsoftDataWiz is made visible.
function ip302p3CsoftDataWiz_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for ip302p3CsoftDataWiz
...
% UIWAIT makes ip302p3CsoftDataWiz wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% Create soft data types pictures
axes(handles.sdAsoftCaxes)
image(imread('guiResources/soft2ex1.png'));
axis off
axes(handles.sdBsoftCaxes)
image(imread('guiResources/soft2ex2.png'));
axis off



% --- Outputs from this function are returned to the command line.
function varargout = ip302p3CsoftDataWiz_OutputFcn(hObject, eventdata, handles)
...
if ~ischar(sdFilename)  % If the user has not provided a soft data file
    ...
    softDataPresent = 1;
    allOk = ip302p3CsoftDataFunction(handles);            
    if allOk
...
function [allOk] = ip302p3CsoftDataFunction(handles)
...
softpdftype = 2;                % BMELIB parameter based on current (2) SD type
...
if isempty(xlsfinfo(sdFilename))  % If it is not Excel format, expect an ASCII file
  ...
  while ~feof(fid)                % Start reading the file to its end
    ...
    if str~=-1                    % If this is not the end of line
      ...
      nLimi = nBins+1;                  % Number of limi values needed for type C SD
      nProbdens = nBins+1;              % Number of required PD values for type C SD
...
% Here, we have softpdftype=2, therefore the proper way to check the area under 
...
if allOk
  for is=1:size(cs,1)
    norm(is,1) = sum(0.5*(probdens(is,1:nl(is)-1)+probdens(is,2:nl(is)))...
                 .*diff(limi(is,1:nl(is))) );                       % For type 2 SD
  end
