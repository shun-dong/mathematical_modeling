function varargout = ip302p3BsoftDataWiz(varargin)
%IP302P3BSOFTDATAWIZ M-file for ip302p3BsoftDataWiz.fig
%      IP302P3BSOFTDATAWIZ, by itself, creates a new IP302P3BSOFTDATAWIZ or raises the existing
%      singleton*.
%
%      H = IP302P3BSOFTDATAWIZ returns the handle to a new IP302P3BSOFTDATAWIZ or the handle to
%      the existing singleton*.
%
%      IP302P3BSOFTDATAWIZ('Property','Value',...) creates a new IP302P3BSOFTDATAWIZ using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to ip302p3BsoftDataWiz_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      IP302P3BSOFTDATAWIZ('CALLBACK') and IP302P3BSOFTDATAWIZ('CALLBACK',hObject,...) call the
%      local function named CALLBACK in IP302P3BSOFTDATAWIZ.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help ip302p3BsoftDataWiz

% Last Modified by GUIDE v2.5 13-Jun-2005 11:15:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ip302p3BsoftDataWiz_OpeningFcn, ...
                   'gui_OutputFcn',  @ip302p3BsoftDataWiz_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before ip302p3BsoftDataWiz is made visible.
function ip302p3BsoftDataWiz_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for ip302p3BsoftDataWiz
...
% UIWAIT makes ip302p3BsoftDataWiz wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% Create soft data types pictures
axes(handles.sdAsoftBaxes)
image(imread('guiResources/soft1ex1.png'));
axis off
axes(handles.sdBsoftBaxes)
image(imread('guiResources/soft1ex2.png'));
axis off



% --- Outputs from this function are returned to the command line.
function varargout = ip302p3BsoftDataWiz_OutputFcn(hObject, eventdata, handles)
...
if ~ischar(sdFilename)  % If the user has not provided a soft data file
    ...
    softDataPresent = 1;
    allOk = ip302p3BsoftDataFunction(handles);            
    if allOk
...
function [allOk] = ip302p3BsoftDataFunction(handles)
...
if isempty(xlsfinfo(sdFilename))  % If it is not Excel format, expect an ASCII file
  ...
  while ~feof(fid)                % Start reading the file to its end
    ...
    if str~=-1                    % If this is not the end of line
      ...
      nLimi = nBins+1;                  % Number of limi values needed for type B SD
      nProbdens = nBins;                % Number of required PD values for type B SD
