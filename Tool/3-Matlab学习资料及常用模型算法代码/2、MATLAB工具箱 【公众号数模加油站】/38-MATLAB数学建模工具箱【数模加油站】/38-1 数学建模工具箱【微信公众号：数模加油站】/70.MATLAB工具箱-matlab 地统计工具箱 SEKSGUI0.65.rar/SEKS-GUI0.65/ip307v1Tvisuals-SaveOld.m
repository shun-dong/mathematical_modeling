...
if KSprocessType==0 | KSprocessType==1 % Initializations in general & BME cases
...
elseif KSprocessType==2                         % Initializations in GBME case
  ...
  errordlg({'ip307v1Tvisuals.m:Preliminary settings:';...
       ['KSprocessType = ' num2str(KSprocessType) ' is anaccounted for.']},...
       'GUI software Error')
end
  
set(handles.graphTypeMenu,'Value',1);                          % Initialize
...
colorScaleMin = [];                                            % Initialize 
colorScaleMax = [];                                            % Initialize 
...
if ~ischar(outFilename)
  ...
  if ~isnan(str2double(textread(outFilename,'%s',1)))
    ...
    if isfield(tempLoad,'validOutputData')   % Check the output file for valid information
      ...
      if KSprocessType==1 % Initializations in BME case
...
if outputDataPresent    % IF outputDataPresent : Then proceed with the user selection
  ...
  if timePresent  % IF timePresent: Allow the slider to function
    ...
    if ~isempty(instanceIncluded)
      ...
      switch get(handles.graphTypeMenu,'Value')
				case 1     % Displays BMEmean. Requires moments or pdf results.
          set(handles.feedbackEdit,'String',...
            'Select a map from the menu to plot at this instance'); 
        case 2     % Displays BMEmean. Requires moments or pdf results.
          mapBmeEstimationMean(handles,tNowActual,tNowOut);          % Create map
        case 3    % Displays BMEmode. Requires mode results
          mapBmeEstimationMode(handles,tNowActual,tNowOut);          % Create map
...
if bmeMom{1,1} | bmePdf{1,1} | bmeCin{1,1} | gbmeMom{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,meanAtk);
    if get(handles.fixColorscaleBox,'Value')
...
if bmeMod{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,modeAtk);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeMom{1,1} | bmePdf{1,1} | bmeCin{1,1} | gbmeMom{1,1}
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
	  ...
	  [trash1,hh] = contourf(xMesh,yMesh,varAtk);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeMom{1,1} | bmePdf{1,1} | bmeCin{1,1} | gbmeMom{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,stdAtk);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeMom{1,1} | bmePdf{1,1} | bmeCin{1,1} | gbmeMom{1,1}
  ...
  if ~(bmeMom{1,1} & applyTransformVisBme & mapInOrigSpaceBme)
    ...
    if totalSpCoordinatesUsed==2
      ...
      [trash1,hh] = contourf(xMesh,yMesh,skewAtk);
      if get(handles.fixColorscaleBox,'Value')
        if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
          set(hh,'LevelList',[colorScaleMin:...
                             (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeCin{1,1}
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,ciSize);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeCin{1,1}
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,ciLowAtk);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeCin{1,1}
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,ciUppAtk);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if bmeCin{1,1}
    ...
    mapDataText = [xMesh(:) yMesh(:) ...
		...
		[trash1,hh] = contourf(xMesh,yMesh,pdfAtCIlim);
    if get(handles.fixColorscaleBox,'Value')
      if ~(isempty(colorScaleMin) | isempty(colorScaleMax))
        set(hh,'LevelList',[colorScaleMin:...
                           (colorScaleMax-colorScaleMin)/5:colorScaleMax]);
...
if gbmeMom{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    [hh] = pcolor(xMesh,yMesh,numuNu);
    caxis([0 2]);
    % Contours are not used: If the value of Nu is the same everywhere, 
    % then the plot will appear blank! We use pcolor for the nu map
    % in the 2 preceding lines, instead.                 
    %[trash1,hh] = contourf(xMesh,yMesh,numuNu);
    ...
    if get(handles.addMaskBox,'Value')==1
      ...
      cd (initialDir)                 % Return to initial directory
		...
		displayString = 'Displaying variable: Spatial order nu of GBME estimations';
    set(handles.feedbackEdit,'String',displayString);
...
if gbmeMom{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    if timePresent
      ...
      displayString = 'Displaying variable: Temporal order mu of GBME estimations';
      set(handles.feedbackEdit,'String',displayString);
...
if gbmeMom{1,1}
  ...
  if totalSpCoordinatesUsed==2
    ...
    if timePresent
      ...
      displayString = 'Displaying variable: Difference of orders nu-mu of GBME estimations';
      set(handles.feedbackEdit,'String',displayString);
