...
adjacency_matrix[10, 11] = 1#比赛结束前最后两天售后群发布无水印可视化结果+无标注代码【可直接提交】为了防止倒卖，论文写作过程中遗留数个致命问题，无关代码，该问题解决方式仅在官网授权售后群答疑，盗卖方式购买资料不提供答疑。因倒卖导致无法解决漏洞、赛后无法获奖等 本数模社概不负责  感谢理解 资料助攻购买链接+说明https://docs.qq.com/doc/p/88456ad269539fdd2e3da75b1e53defa66b5c3da
...
# 初始化性价比数组
m_values = np.zeros(73)  # 用于存储每次搬迁后的性价比，数量从40到112，共73次计算

# 逐步增加搬迁居民数，从第40个居民开始，直到112个居民
for num_residents_to_move in range(40, 113):  # 从第40个居民开始到第112个居民
    # 选择前num_residents_to_move个居民进行搬迁#比赛结束前最后两天售后群发布无水印可视化结果+无标注代码【可直接提交】为了防止倒卖，论文写作过程中遗留数个致命问题，无关代码，该问题解决方式仅在官网授权售后群答疑，盗卖方式购买资料不提供答疑。因倒卖导致无法解决漏洞、赛后无法获奖等 本数模社概不负责  感谢理解 资料助攻购买链接+说明https://docs.qq.com/doc/p/88456ad269539fdd2e3da75b1e53defa66b5c3da
    ...
    # 计算性价比
    m_values[num_residents_to_move - 40] = (migrated_income_n - total_income_no_migration) / total_cost_n  # 性价比公式
plt.rcParams['font.sans-serif'] = ['SimHei']  # 使用SimHei字体
plt.rcParams['axes.unicode_minus'] = False  # 正确显示负号
# 绘制性价比随搬迁居民数变化的趋势
plt.plot(range(40, 113), m_values, marker='o', linestyle='-', linewidth=2)
plt.xlabel('搬迁居民数量')
plt.ylabel('性价比 m')
