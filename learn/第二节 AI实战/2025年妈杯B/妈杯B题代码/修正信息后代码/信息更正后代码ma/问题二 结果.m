...
disp(['总时间损失收入：', num2str(total_time_loss_income)]);




% 计算最终的收入（扣除时间损失）
total_income = total_income - time_loss_income;

% 计算最终的盈利（扣除总成本）
total_profit = total_income - total_cost;  % 总收入减去总成本

% 输出投资成本
disp(['总投资成本（搬迁补偿 + 沟通成本）：', num2str(total_cost(1))]);
disp(['总收入：', num2str(total_income)]);  % 输出收入
