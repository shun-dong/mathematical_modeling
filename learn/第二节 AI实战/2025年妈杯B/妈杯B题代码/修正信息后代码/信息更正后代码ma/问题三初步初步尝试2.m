...
adjacency_matrix(24, 25) = 1;%比赛结束前最后两天售后群发布无水印可视化结果+无标注代码【可直接提交】为了防止倒卖，论文写作过程中遗留数个致命问题，无关代码，该问题解决方式仅在官网授权售后群答疑，盗卖方式购买资料不提供答疑。因倒卖导致无法解决漏洞、赛后无法获奖等 本数模社概不负责  感谢理解 资料助攻购买链接+说明https://docs.qq.com/doc/p/88456ad269539fdd2e3da75b1e53defa66b5c3da
...
% 初始化性价比数组
m_values = zeros(73, 1);  % 用于存储每次搬迁后的性价比，数量从40到112，共73次计算

% 逐步增加搬迁居民数，从第40个居民开始，直到112个居民
for num_residents_to_move = 40:112  % 从第40个居民开始到第112个居民
    ...
    % 计算性价比
    m_values(num_residents_to_move - 39) = (migrated_income_n - total_income_no_migration) / total_cost_n;  % 性价比公式
end

% 绘制性价比随搬迁居民数变化的趋势
figure;
plot(40:112, m_values, '-o', 'LineWidth', 2);
xlabel('搬迁居民数量');
ylabel('性价比 m');
